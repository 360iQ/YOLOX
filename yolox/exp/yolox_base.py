...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.act: Literal["silu", "relu"] = "silu"
        ...
        self.iou_loss_type: Literal["iou", "silu", "diou", "ciou", "giou"] = "iou"

        # ---------------- dataloader config ---------------- #
        # set worker to 4 for shorter dataloader init time
        # If your training process cost many memory, reduce this value.
        self.data_num_workers: int = 4
        self.input_size: tuple[int, int] = (640, 640)  # (height, width)
        # Actual multiscale ranges: [640 - 5 * 32, 640 + 5 * 32].
        # To disable multiscale training, set the value to 0.
        self.multiscale_range: int = 5
        ...
        self.mosaic_prob: float = 1.0
        # prob of applying mixup aug
        self.mixup_prob: float = 1.0
        # prob of applying hsv aug
        self.hsv_prob: float = 1.0
        # prob of applying flip aug
        self.flip_prob: float = 0.5
        # rotation angle range, for example, if set to 2, the true range is (-2, 2)
        self.degrees: float = 10.0
        # translate range, for example, if set to 0.1, the true range is (-0.1, 0.1)
        self.translate: float = 0.1
        self.mosaic_scale: tuple[float, float] = (0.1, 2)
        # apply mixup aug or not
        self.enable_mixup: bool = True
        self.mixup_scale: tuple[float, float] = (0.5, 1.5)
        # shear angle range, for example, if set to 2, the true range is (-2, 2)
        self.shear: float = 2.0
        ...
        self.warmup_lr: float = 0
        self.min_lr_ratio: float = 0.05
        # learning rate for one image. During training, lr will multiply batchsize.
        self.basic_lr_per_img: float = 0.01 / 64.0
        # name of LRScheduler
        self.scheduler = "yoloxwarmcos"
        # last #epoch to close augmention like mosaic
        self.no_aug_epochs: int = 15
        ...
        self.print_interval: int = 10
        # eval period in epoch, for example,
        # if set to 1, model will be evaluate after every epoch.
        self.eval_interval: int = 10
        # save history checkpoint or not.
        # If set to False, yolox will only save latest and best ckpt.
        self.save_history_ckpt: bool = True
        # name of experiment
        self.exp_name: str = os.path.split(os.path.realpath(__file__))[1].split(".")[0]

        # -----------------  testing config ------------------ #
        # output image size during evaluation/test
        self.test_size: tuple[int, int] = (640, 640)
        # confidence threshold during evaluation/test,
        # boxes whose scores are less than test_conf will be filtered
        self.test_conf: float = 0.01
        # nms threshold
        self.nmsthre: float = 0.65
        # freeze model backbone weights
        self.freeze_backbone: bool = False
        self.single_image_transforms = None
        self.class_agnostic_nms_valid: bool = False
